...
def Energiekalibration(xdata,dxdata):
    # Bestimme Gerade aus den zwei Punkten der Erwartungswerte der Linien, die wir auf die Energie der Linien mappen k�nnen.
    # Bestimme Steigung als rise over run (mittlere �nderungsrate)
    m= (5.775-2.892)/(xdata[0]-xdata[1])
    dm=(5.775-2.892)/(xdata[0]-xdata[1])**2 *np.sqrt(dxdata[0]**2 +dxdata[1]**2)

    # Bestimme y-Achsenabschnitt als Offset f�r eine Linie
    b= 5.775-m* xdata[0]
    db=np.sqrt((xdata[0]*dm)**2 +(dxdata[0]*m)**2)
    return [m,b],[dm,db]

def calibrationfunction(x,m,b,dm,db):
    # Baue eine Gerade mit diesen Parametern und den dazugeh�rigen Fehlern
